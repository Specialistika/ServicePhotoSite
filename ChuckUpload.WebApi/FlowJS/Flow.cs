namespace ChuckUpload.WebApi.Flow
{
    using Context;
    using Reader;
    using Request;
    ...
    public class Flow
    {
        ...
        {
            return string.Concat(context.GetChunkPathFunc(request), "/",context.GetChunkFileName(request));
        }

        private async Task<HttpResponseMessage> HandlePostRequest(FlowRequestContext context)
        {
            // read request 
            FlowRequest request = await _requestReader.ReadPostAsync(context, _fileSystem).ConfigureAwait(false);
            ...
            {
               nameFile = await CombineAsync(context, request).ConfigureAwait(false);
            }

			return context.HttpRequest.CreateResponse(HttpStatusCode.OK, nameFile);
        }

        private async Task<string> CombineAsync(FlowRequestContext context, FlowRequest request)
        {
            var filePath = string.Concat( context.GetFilePathFunc(request), "/", context.GetFileName(request));
